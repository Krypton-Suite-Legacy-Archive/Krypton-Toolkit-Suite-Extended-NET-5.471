            ...
            set
            {
                if (value != null)
                {
                    ...
                    if (useParentRelationship)
                    {
                        ...
                        //Init nodes
                        //-------------------------- 
                        BuildPortsNodesInitialLoop("", dataSource, this.iDColumnName, this._parentIDColumnName, 0);
                    }
                    else
                    {
                        //-------------------------- 
                        //Setup font
                        //-------------------------- 
                        Font boldFont = new Font(this.DefaultCellStyle.Font, FontStyle.Bold);
                        ...
                        foreach (DataRow dr in dataSource.Rows)
                        {
                            ...
                            if (isChild)
                            {
                                if (isOneLevel)
                                {
                                    ...
                                    //set bold
                                    //-------------------------- 
                                    if (fontParentBold) parentNode.DefaultCellStyle.Font = new Font(this.DefaultCellStyle.Font, FontStyle.Bold);
                            ...
                            else
                            {
                                if (isOneLevel)
                                {
                                    ...
                                    parentNode = this.Nodes.Add(p);
                                    parentNode.ImageIndex = imageIndexChild;
                                }
                                else
                                {
                                    node = new TreeGridNode();
                                    node = this.Nodes.Add(p);
                                    node.ImageIndex = imageIndexParent;
                                    //-------------------------- 
                                    //set bold
                                    //-------------------------- 
                                    if (fontParentBold) node.DefaultCellStyle.Font = new Font(this.DefaultCellStyle.Font, FontStyle.Bold);
            ...
            if (dv.Count > 0)
            {
                try
                {
                    for (NetItems = 0; NetItems <= dv.Count - 1; NetItems++)
                    {
                        ...
                        TreeGridNode RootNode = this.Nodes.GetNodeByUniqueValue(UniqueId);

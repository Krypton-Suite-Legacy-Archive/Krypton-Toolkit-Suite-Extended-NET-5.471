    ...
    public class ColourWheel : IDisposable
    {
        ...
        public void Draw(Graphics g, Point mousePoint)
        {
            ...
            if (currentState == MouseState.MouseUp)
            {
                if (!mousePoint.IsEmpty)
                {
                    if (colourRegion.IsVisible(mousePoint))
                    {
                        ...
                        currentState = MouseState.ClickOnColour;
                    }
            ...
            switch (currentState)
            {
                ...
                case MouseState.ClickOnColour:
                case MouseState.DragInColour:
                    // Calculate new color information
                    ...
                    if (currentState == MouseState.DragInColour)
                    {
                        if (distance > 1)
                        {
            ...
            switch (currentState)
            {
                case MouseState.ClickOnBrightness:
                    ...
                    break;
                case MouseState.ClickOnColour:
                    currentState = MouseState.DragInColour;
                    break;
        ...
        private void DrawLinearGradient(Color TopColour)
        {
            ...
            using (LinearGradientBrush lgb =
                             new LinearGradientBrush(brightnessRectangle, TopColour,
        ...
        private Color[] GetColors()
        {
            ...
            for (int i = 0; i <= COLOR_COUNT - 1; i++)
                Colors[i] = ColourHandler.HSVtoColour((int)((double)(i * 255) / COLOR_COUNT), 255, 255);
            return Colors;
        }

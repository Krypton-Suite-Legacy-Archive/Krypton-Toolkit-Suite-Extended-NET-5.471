...
namespace ExtendedFileDialogs.Controls
{
    ...
    Designer("Designer.KryptonTaskDialogDesigner")]
    public partial class TaskDialog : Component, IWin32Window
    {
        ...
        private Enumerations.ProgressBarState _progressBarState = Enumerations.ProgressBarState.Normal;
        ...
        /// </note>
        /// </remarks>
        [Category("Behavior"), Description("The type of progress bar displayed on the dialog."), DefaultValue(Enumerations.ProgressBarStyle.None)]
        public Enumerations.ProgressBarStyle ProgressBarStyle
        {
            get
            {
                if (GetFlag(NativeMethods.TaskDialogFlags.ShowMarqueeProgressBar))
                    return Enumerations.ProgressBarStyle.MarqueeProgressBar;
                else if (GetFlag(NativeMethods.TaskDialogFlags.ShowProgressBar))
                    return Enumerations.ProgressBarStyle.ProgressBar;
                else
                    return Enumerations.ProgressBarStyle.None;
            }
            set
            {
                SetFlag(NativeMethods.TaskDialogFlags.ShowMarqueeProgressBar, value == Enumerations.ProgressBarStyle.MarqueeProgressBar);
                SetFlag(NativeMethods.TaskDialogFlags.ShowProgressBar, value == Enumerations.ProgressBarStyle.ProgressBar);
        ...
        private void UpdateProgressBarStyle()
        {
            if (IsDialogRunning)
            {
                NativeMethods.SendMessage(Handle, (int)NativeMethods.TaskDialogMessages.SetMarqueeProgressBar, new IntPtr(ProgressBarStyle == Enumerations.ProgressBarStyle.MarqueeProgressBar ? 1 : 0), IntPtr.Zero);
            }
        }

    ...
    public class VistaSaveFileDialog : VistaFileDialog
    {
        ...
        protected override void OnFileOk(CancelEventArgs e)
        {
            ...
            if (DownlevelDialog == null)
            {
                ...
                {
                    PromptUser(ComDlgResources.FormatString(ComDlgResourceId.FileNotFound, Path.GetFileName(FileName)), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    e.Cancel = true;
                    return;
                }
                if (CreatePrompt && !File.Exists(FileName))
                {
                    if (!PromptUser(ComDlgResources.FormatString(ComDlgResourceId.CreatePrompt, Path.GetFileName(FileName)), MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation))
                    {
                        e.Cancel = true;
                        return;
                    }
